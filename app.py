...
def display_daily_view(df, year, month):
    ...
    st.markdown('<div class="daily-view-container">', unsafe_allow_html=True)
    ...
    for day in range(1, num_days_in_month + 1):
        ...
        # Start a day column
        st.markdown(f'<div class="day-column"><h4>اليوم {day} <small>({weekday_name})</small></h4>', unsafe_allow_html=True)
        
        if day_df.empty:
            st.markdown("<p><i>لا توجد مناوبات</i></p>", unsafe_allow_html=True)
        else:
